    ...
    form.patch(route('profile.update'), {
        ...
        onSuccess: () => {
            toast.add({
                ...
                detail: trans('profile.updated_detail'),
                life: 3000,
            });
        },
        onError: () => {
            toast.add({
                ...
                detail: trans('profile.update_failed_detail'),
                life: 4000,
            });
        },
    });
...
const notifyVerificationSent = () => {
    toast.add({
        severity: 'success',
        summary: trans('auth.verify_email.link_sent'),
        detail: trans('auth.verify_email.link_sent'),
        life: 3000,
    });
};
</script>
    ...
    <section class="space-y-5">
        <div class="grid gap-4">
            <div class="grid gap-2">
                <label for="profile-name" class="text-sm font-medium text-slate-700">{{ trans('fields.name') }}</label>
                <InputText
                    id="profile-name"
                    v-model="form.name"
                    autocomplete="name"
            ...
            <div class="grid gap-2">
                <label for="profile-email" class="text-sm font-medium text-slate-700">{{ trans('auth.email') }}</label>
                <InputText
                    id="profile-email"
            ...
            >
                <div class="text-sm font-semibold text-amber-950">{{ trans('auth.verify_email.eyebrow') }}</div>
                ...
                >
                    {{ trans('auth.verify_email.resend_cta') }}
                </Link>
